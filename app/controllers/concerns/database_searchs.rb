module DatabaseSearchs
  ...
  def fetch_last_month
    Report.distinct.where(year: Time.current.year).order('month_numb').pluck(:month_numb)[-1]
  end
  ...
  def fetch_report_by_next_month(current_report)
    Report.where(month_numb: current_report.month_numb+1)[0]
  rescue StandardError
    nil
  end
  ...
  #def fetch_report_by_month_year(month, year)
    #Report.where(month: month, year: year)[0]
  #end
